...
**方案 A（每次手动，更安全）**：搜索前执行 `msedge --remote-debugging-port=9223`

**方案 B（一劳永逸）**：右键 Edge 快捷方式 → 属性 → 目标末尾加 ` --remote-debugging-port=9223`，之后每次打开 Edge 都自带 CDP。

> **设计说明**：CDP 仅监听 127.0.0.1，外部网络不可达。Agent 不自动启动浏览器，不越权，不杀进程。两种方案由用户自选。
