...
**最小设计原则：Agent 不自动启动浏览器。用户手动按需开关 CDP，用完即关。**

### Agent 检查流程

Agent 连接 http://127.0.0.1:9222/json/version 检测 CDP：
- **通** → 直接进入搜索流程
- **不通** → 输出以下指引，等用户确认后重试：

> 请关闭当前 Edge 浏览器，然后在运行窗口（Win+R）或终端执行：
> `
...
| 风险 | 说明 |
|------|------|
| CDP 无认证 | 仅监听 127.0.0.1，外网不可达。但同机任意进程可无认证操控浏览器 |
| 后台进程残留 | Edge 默认关闭窗口后后台进程仍运行，CDP 端口不会自动释放。需在 edge://settings/system 关闭「在 Microsoft Edge 关闭后继续运行后台扩展和应用」，否则需手动杀进程 |
| **解决方案** | 按需开关：搜前开 → 搜完关（并确认进程退出）。不设开机自启，不设快捷方式，不放启动文件夹。 |

### 为什么不自动启动

沙箱会阻止 Edge 写注册表（ACCESS_DENIED），自动启动必然失败。打包 exe / bat / 快捷方式均无法绕过——沙箱拦截的是 Edge 二进制自身行为，非启动方式。
