...
| `ECONNREFUSED 127.0.0.1:9223` | 沙箱网络隔离 或 浏览器未开调试端口 | 先确认 CDP 端口可用（`curl http://127.0.0.1:9223/json/version`），不可用则告知用户启动浏览器（见 CDP 前置章节） |
...
| CDP 连接 localhost:9223 | 被拦截 | 沙箱网络隔离 |
...
本 Skill 需要浏览器开启远程调试端口（默认 9223，自动扫描 9223-9226）。

**设计原则：Agent 不自动启动浏览器，不越权，不杀进程。两个方案由用户自选。**

### Agent 行为

连接 http://127.0.0.1:9223/json/version：
- **通** → 进入搜索
- **不通** → 输出：

> CDP 不可用。请二选一：
> **A（每次手动，更安全）**：执行 `msedge --remote-debugging-port=9223`，然后告诉我继续。
> **B（一劳永逸）**：右键 Edge 快捷方式 → 属性 → 目标末尾加 ` --remote-debugging-port=9223`（注意前面有空格）。之后每次打开 Edge 都自带 CDP。
...
`<PYTHON>` = `PYTHONUTF8=1 PYTHONIOENCODING=utf-8 local_env.python_venv`。`{SKILL_DIR}` = 本 skill 目录。
